...
class PiperProcessManager:
    ...
    async def synthesize_text(
        ...
        """Synthesize text using piper and return the output file path."""
        if voice_name is None:
            voice_name = self.args.voice
        
        if voice_name == self.args.voice:
            voice_speaker = voice_speaker or self.args.speaker
        ...
        try:
            # Escape and clean text for command line
            # Remove newlines and normalize whitespace
            escaped_text = " ".join(text.strip().split())
            ...
            piper_args = [
                ...
                "--output_dir",
                wav_dir,
                escaped_text,  # Text as command line argument
            ]
            ...
            _LOGGER.debug(
                "Running piper synthesis: %s args=%s", self.args.piper, piper_args
            )
            
            # Run piper process for this synthesis
            proc = await asyncio.create_subprocess_exec(
                self.args.piper,
                *piper_args,
                stderr=asyncio.subprocess.PIPE,
            ...
            if returncode != 0:
                ...
                raise RuntimeError(f"Piper process failed with code {returncode}: {stderr_output}")
            
            # Find the output file in the wav_dir
            wav_files = list(Path(wav_dir).glob("*.wav"))
            if not wav_files:
                raise RuntimeError("No WAV file generated by piper")
            
            # Move the file to a location where it won't be deleted immediately
            import shutil
            output_file = tempfile.NamedTemporaryFile(suffix=".wav", delete=False)
            output_file.close()
            shutil.move(str(wav_files[0]), output_file.name)
